...
const Checkout = () => {
    ...
    const handlePlaceOrder = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(order.error || 'Could not place the order.');
            }
            ...
            setOrderError(error instanceof Error ? error.message : 'Could not place the order.');
        } finally {
            ...
            <main className='relative bg-white dark:bg-dark-bg transition-colors duration-300'>
                ...
                <section className='padding-x py-10 w-full min-h-screen flex flex-col justify-center items-center pt-28 text-center px-4'>
                    ...
                    <p className='mt-4 font-montserrat text-lg text-slate-gray dark:text-dark-muted'>
                        Thank you for your purchase. Your order number is <span className='font-semibold text-coral-red'>{orderNumber}</span>.
                    </p>
                    <p className='mt-2 max-w-xl font-montserrat text-sm leading-6 text-slate-gray dark:text-dark-muted'>
                        Your order has been saved by the Next.js API. Connect Stripe or PayPal before accepting online card payments.
                    </p>
            ...
            <section className='padding-x py-10 w-full min-h-screen pt-28'>
                ...
                <div className='flex-1'>
                    ...
                    <form onSubmit={handlePlaceOrder} className='flex flex-col gap-4'>
                        {orderError && (
                            <div className='rounded-2xl border border-red-200 bg-red-50 px-5 py-4 font-montserrat text-sm text-red-600 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400'>
                                {orderError}
                            </div>
                        )}
                        <input
                        ...
                        <div className='flex flex-col sm:flex-row gap-4'>
                            <input
                                ...
                                className='flex-1 border border-slate-gray dark:border-dark-border rounded-full px-5 py-4 font-montserrat text-base text-slate-gray dark:text-dark-muted outline-none bg-transparent dark:bg-dark-card transition-colors'
                            />
                            <input
                                type='text'
                        ...
                        >
                            ...
                            <option>Card payment - requires Stripe integration</option>
                            <option>PayPal - requires PayPal integration</option>
                        </select>
                        <div className='mt-6'>
                            <Button label={isSubmitting ? 'Placing Order...' : 'Place Order'} fullWidth disabled={isSubmitting} />
                        </div>
                    </form>
                </div>
